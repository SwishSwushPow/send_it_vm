...
use crate::config::{GUEST_WORKSPACE, VmSettings};
...
/// `share <tag> <ro|rw> <guest path>` and `hide <guest path>`.
fn manifest(settings: &VmSettings, shares: &[Share]) -> Result<String> {
    let mut text = String::from("# Generated by send_it; applied by mount.sh on boot.\n");
    ...
    if !settings.expose_git {
        writeln!(text, "hide {GUEST_WORKSPACE}/.git")?;
    }
    Ok(text)
}
...
mod tests {
    ...
    fn builds_shares_and_manifest() {
        ...
        assert_eq!(
            ...
            "# Generated by send_it; applied by mount.sh on boot.\n\
             share project rw /workspace\n\
